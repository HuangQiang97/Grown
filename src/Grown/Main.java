...
import Grown.DPFGrown;
import Grown.DivideClass;
import Grown.GetThreshold;
import Grown.Util;
...
public class Main {
    public static void main(String[] args) {
        //输入图像文件路径。
        String filePath="inputImage\\Pic_3.jpg";
        //获得输入图像的彩色像素点二维数组。
        ...
        //  BufferedImage newoutputImage=new BufferedImage(width,height,BufferedImage.TYPE_3BYTE_BGR);
        ...
        byte[][]sameClassPeocessMap=new byte[height][width];
        //对后加入的种子点，他们无同一类种子栈，要在初始种子点中遍历，记录下初始链表长度。
        int initSize=tempSeedList.size();
        int []pricessInitSeed=new int[initSize];

        //遍历种子链表进行生长，并不断加入新的种子点。
        DPFGrown dpfGrown=new DPFGrown(0xff0000,colorArray,sameClassPeocessMap,tempSeedList,outputImage);
        for (int i=0; i<tempSeedList.size();i++) {
...
//解决方法二：鸵鸟算法，不处理，在进行生长时同一点会被跳过。
                    ...
                    if (pricessInitSeed[j] == 0) {
                        ...
                        for (Seed sameClassSeed : tempSeedList.get(j).sameClassSeed
                            ...
                            if (sameClassPeocessMap[sameClassSeed.getX()][sameClassSeed.getY()] == 0) {
                                ...
                                // if (tempSeedList.indexOf(sameClassSeed) == -1) {
                                //新加入点未设置阈值，在这里设置阈值。
                                sameClassSeed.setBestThreshhold((int) tempSeedList.get(i).getBestThreshold());
                                tempSeedList.add(sameClassSeed);
        ...
        for (Seed seed :tempSeedList
            ...
            for (Map.Entry<Seed, Integer> map : hashMap.entrySet()
            ) {
                if (seed == map.getKey()) {
                    hashMap.put(seed, map.getValue() + 1);
                }
            }
        }

        System.out.println("最终种子数："+hashMap.size());
        //输出最终结果。
        try {
            ImageIO.write(outputImage,"png",new File("Result\\grownPicture\\ "+date.getHours()+date.getMinutes()+".jpg"));
        } catch (IOException e) {
            e.printStackTrace();
